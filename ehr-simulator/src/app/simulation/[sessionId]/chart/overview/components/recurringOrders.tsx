
'use client'

import { Tooltip } from "@/components/ui/tooltip"
import { Card, CardContent } from "@/components/ui/card"
import { Info } from "lucide-react"
import { TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip"
import StyledTitle from "./styledTitle"
import { useSimulationTime } from "../../context/SimulationTimeContext"
const RecurringOrders = () => {
  const { orders } = useSimulationTime()
  const nursingOrders = orders.filter(
    (order) => order.category === "Nursing" && Boolean(order.is_important)
  )
  const laboratoryOrders = orders.filter(
    (order) => order.category === "Laboratory" && Boolean(order.is_important)
  )

  return (
    <Card className="relative col-span-1 pt-2 overflow-hidden h-fit gap-3">
      <StyledTitle color="bg-sky-200" firstLetter="R" secondLetter="ecurring Orders" />
      <CardContent className="grid gap-4 px-8">
        <div className="flex flex-col w-full items-start gap-1">
          <div className="flex flex-col gap-2">
            <p className="text-sm font-medium leading-none">Nursing</p>
            {nursingOrders.map(order => {
              return (
                <div key={order.id} className="flex pl-2 gap-3 items-center">
                  <p className="text-xs text-neutral-500 tracking-tight">{order.title}</p>
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger>
                        <Info size={14} color="#d1d5db" />
                      </TooltipTrigger>
                      <TooltipContent className="w-fit">
                        <p className="max-w-120  text-wrap">{order.details}</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
              )
            })}

            <p className="text-sm font-medium leading-none">Labs</p>
            {laboratoryOrders.map(order => {
              return (
                <div key={order.id} className="flex pl-2 gap-3 items-center">
                  <p className="text-xs text-neutral-500 tracking-tight">{order.title}</p>
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger>
                        <Info size={14} color="#d1d5db" />
                      </TooltipTrigger>
                      <TooltipContent className="w-fit">
                        <p className="max-w-120  text-wrap">{order.details}</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
              )
            })}
            {nursingOrders.length === 0 && laboratoryOrders.length === 0 ? (
              <p className="text-xs text-neutral-500 tracking-tight">No recurring orders documented.</p>
            ) : null}
          </div>
        </div>
      </CardContent>
      <div className="absolute bottom-0 bg-sky-200 w-full h-3"></div>
    </Card>
  )
}

export default RecurringOrders
