...
export default function MicrobiologyReport({ report, cellLabel }: PathologyReportProps) {
  ...
  return (
    <Dialog>
      ...
      >
        {cellLabel}{report.isCritical && <OctagonAlert size={16} color="#e60000" />}
      </DialogTrigger>
      <DialogContent className="p-4 sm:max-w-3/4 xl:max-w-4xl overflow-y-auto gap-0">
        <div className="flex justify-between w-full">
          <DialogTitle className="flex h-fit items-baseline gap-2 pb-2">
            {report.isCritical && <OctagonAlert size={18} color="#e60000" />}{report.sampleType}
        ...
        <div className="grid gap-1">
          ...
          <div>
            <div className="flex gap-1">
              ...
              {report.isCritical && <p className="text-sm font-bold text-red-500">!!</p>}
            </div>
            <p className="flex pl-2 text-sm text-left text-gray-800 tracking-tight">
              {report.cultureResults}
            </p>
            <Separator className="my-1" />
          </div>
          <div>
